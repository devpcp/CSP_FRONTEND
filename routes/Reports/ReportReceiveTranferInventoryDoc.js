...
const { TabPane } = Tabs;
const ReportReceiveTranferInventoryDoc = () => {
    ...
    */
    ...
    const getDataSearch = async ({ search = modelSearch.search ?? "", limit = configTable.limit, page = configTable.page, sort = configSort.sort, order = (configSort.order === "descend" ? "desc" : "asc"), _status = modelSearch.status, documentdate = modelSearch.documentdate ,businessPartner = modelSearch.businessPartner ?? [] }) => {
        try {
            ...
            let url = `/shopReports/inventory?limit=${limit}&page=${page}&sort=${sort}&order=${order}&search=${search}&start_date=${start_date}&end_date=${end_date}&doc_type_id=4979e859-92d1-4485-9243-45cdd505adb8`
    ...
    const exportExcel = async () => {
        setLoading(true)
        const res = await API.get(`/shopReports/inventory?export_format=xlsx&start_date=${startDate}&end_date=${endDate}&doc_type_id=4979e859-92d1-4485-9243-45cdd505adb8`)
...
export default ReportReceiveTranferInventoryDoc
