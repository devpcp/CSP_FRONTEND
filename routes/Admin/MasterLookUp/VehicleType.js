...
const VehicleType = () => {
    ...
    const setColumnsTable = () => {
        const _column = [
            ...
            {
                title: GetIntlMessages("vehicle-type-name"),
                dataIndex: '',
                key: '',
                width: 250,
    ...
    const changeStatus = async (isuse, id) => {
        try {
            ...
            const { data } = await API.put(`/master/vehicleType/put/${id}`, { status })
    ...
    const addEditViewModal = async (mode, id) => {
        ...
        try {
            ...
            if (id) {
                setIsIdEdit(id)
                const { data } = await API.get(`/master/vehicleType/byid/${id}`)
    ...
    const onFinish = async (value) => {
        try {
            ...
            if (configModal.mode === "add") {
                res = await API.post(`/master/vehicleType/add`, model)
            } else if (configModal.mode === "edit" && idEdit) {
                model.status = checkedIsuse ? "active" : "block"
                res = await API.put(`/master/vehicleType/put/${idEdit}`, model)
            }
            if (res.data.status == "success") {
                callback()
            } else {
        ...
        <>
            <>
                ...
                <ModalFullScreen
                    // width={750}
                    maskClosable={false}
                    title={`${configModal.mode == "view" ? GetIntlMessages("view-data") : configModal.mode == "edit" ? GetIntlMessages("edit-data") : GetIntlMessages("add-data")} ${GetIntlMessages("vehicle-type")}`}
                ...
                >
                    ...
                    >
                        ...
                        >
                            <Input disabled={configModal.mode != "Edit"} />
                        </Form.Item>
                        ...
                        <FormInputLanguage icon={formLocale} label={GetIntlMessages("vehicle-type-name")} disabled={configModal.mode == "view"} name="type_name" rules={[{ required: true, message: GetIntlMessages("please-fill-out") }]} />
