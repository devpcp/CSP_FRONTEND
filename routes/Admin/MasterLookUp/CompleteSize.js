...
const CompleteSize = () => {
    ...
    const setColumnsTable = () => {
        const _column = [
            ...
            {
                title: GetIntlMessages("complete-size"),
                dataIndex: '',
                key: '',
                width: 250,
                render: (text, record) => _.get(text, `complete_size_name[${locale.locale}]`, "-"),
    ...
    const getDataSearch = async ({ search = modelSearch.search ?? "", limit = configTable.limit, page = configTable.page, sort = configSort.sort, order = (configSort.order === "descend" ? "desc" : "asc"), _status = modelSearch.status }) => {
        try {
            ...
            const res = await API.get(`/productCompleteSize/all?search=${search}&sort=${sort}&order=${order}`)
            console.log('res', res)
    ...
    const changeStatus = async (isuse, id) => {
        try {
            ...
            const { data } = await API.put(`/productCompleteSize/put/${id}`, { status })
            // console.log('data', data)
    ...
    const addEditViewModal = async (mode, id) => {
        ...
        try {
            ...
            if (id) {
                setIsIdEdit(id)
                const { data } = await API.get(`/productCompleteSize/byid/${id}`)
    ...
    const onFinish = async (value) => {
        try {
            ...
            if (configModal.mode === "add") {
                ...
                res = await API.put(`/productCompleteSize/put/${idEdit}`, model)
            }
            if (res.data.status == "successful") {
                callback()
            } else {
    ...
    return (
        <>
            <>
                ...
                <ModalFullScreen
                    maskClosable={false}
                    title={`${configModal.mode == "view" ? GetIntlMessages("view-data") : configModal.mode == "edit" ? GetIntlMessages("edit-data") : GetIntlMessages("add-data")} ${GetIntlMessages("complete-size")}`}
                ...
                >
                    ...
                    >
                        ...
                        <FormInputLanguage icon={formLocale} label={GetIntlMessages("complete-size")} disabled={configModal.mode == "view"} name="complete_size_name" rules={[{ required: true, message: GetIntlMessages("please-fill-out") }]} />
