...
const NameTitle = () => {
    ...
    const setColumnsTable = () => {
        const _column = [
            ...
            {
                title: `${GetIntlMessages("name-title")}`,
                dataIndex: '',
                key: '',
                width: 250,
                render: (text, record) => _.get(text, `name_title[${locale.locale}]`, "-"),
    ...
    const changeStatus = async (isuse, id) => {
        try {
            ...
            const { data } = await API.put(`/master/nameTitle/put/${id}`, { status })
    ...
    const addEditViewModal = async (mode, id) => {
        ...
        try {
            ...
            if (id) {
                setIsIdEdit(id)
                const { data } = await API.get(`/master/nameTitle/byid/${id}`)
    ...
    const onFinish = async (value) => {
        try {
            ...
            if (configModal.mode === "add") {
                res = await API.post(`/master/nameTitle/add`, model)
            } else if (configModal.mode === "edit" && idEdit) {
                model.status = checkedIsuse ? "active" : "block"
                res = await API.put(`/master/nameTitle/put/${idEdit}`, model)
            }
            if (res.data.status == "success") {
                callback()
            } else {
    ...
    return (
        <>
            <>
                ...
                <ModalFullScreen
                    maskClosable={false}
                    title={`${configModal.mode == "view" ? GetIntlMessages("view-data") : configModal.mode == "edit" ? GetIntlMessages("edit-data") : GetIntlMessages("add-data")} ${GetIntlMessages("name-title")}`}
                ...
                >
                    ...
                    >
                        ...
                        <FormInputLanguage icon={formLocale} label={GetIntlMessages("name-title")} disabled={configModal.mode == "view"} name="name_title" rules={[{ required: true, message: GetIntlMessages("please-fill-out") }]} />

                        <FormInputLanguage icon={formLocale} label={GetIntlMessages("name-title-acronyms")} disabled={configModal.mode == "view"} name="initials"/>
