    ...
    */
    ...
    const setColumnsTable = () => {
        const _column = [
                ...
                render: (text, record, index) => {
                    try {
                        if (record.bus_customer_id !== null) {
                            return record.ShopBusinessCustomer.customer_name
                        }
                        if (record.per_customer_id !== null) {
                            return record.ShopPersonalCustomer.customer_name
                        }
                    } catch (error) {
                        return ""
                    }
            ...
            },
            {
                ...
                align: "center",
                use: true,
                render: (text, record) => get(text, `payment_price_paid` ?? "-", "-").toLocaleString()
            },
            {
                title: 'คงเหลือ',
                ...
                use: true,
                render: (text, record) => {
                    return (record.price_grand_total - record.PaymentInfo.payment_price_paid).toLocaleString()
                }
            },
            {
    ...
    const getDataSearch = async ({ search = modelSearch.search ?? "", limit = configTable.limit, page = configTable.page, sort = configSort.sort, order = (configSort.order === "descend" ? "desc" : "asc"), _status = modelSearch.status, doc_date = modelSearch.doc_date, debt_due_date = modelSearch.debt_due_date, select_shop_ids = modelSearch.select_shop_ids ?? [], payment_paid_status = modelSearch.payment_paid_status ?? [] }) => {
        try {
            ...
            let url = `/shopReports/customerDebt?limit=${limit}&page=${page}&sort=${sort}&order=${order}&search=${search}${doc_date_startDate !== "" ? `&doc_date_startDate=${doc_date_startDate}` : ""}${doc_date_endDate !== "" ? `&doc_date_endDate=${doc_date_endDate}` : ""}${debt_due_date_startDate !== "" ? `&debt_due_date_startDate=${debt_due_date_startDate}` : ""}${debt_due_date_endDate !== "" ? `&debt_due_date_endDate=${debt_due_date_endDate}` : ""}${select_shop_ids !== "" ? `&select_shop_ids=${select_shop_ids}` : ""}${payment_paid_status !== "" ? `&payment_paid_status=${payment_paid_status}` : ""}${_status !== "" ? `&status=${_status}` : ""}`
    ...
    //     setLoading(true)
    //     const res = await API.get(`/customerDebt/all?export_format=xlsx&start_date=${startDate}&end_date=${endDate}`)
    ...
    const exportExcel = async () => {
        ...
        try {
            ...
            const res = await API.get(`/shopReports/customerDebt?limit=999999&page=1&search=${search}${doc_date_startDate !== "" ? `&doc_date_startDate=${doc_date_startDate}` : ""}${doc_date_endDate !== "" ? `&doc_date_endDate=${doc_date_endDate}` : ""}${debt_due_date_startDate !== "" ? `&debt_due_date_startDate=${debt_due_date_startDate}` : ""}${debt_due_date_endDate !== "" ? `&debt_due_date_endDate=${debt_due_date_endDate}` : ""}${select_shop_ids !== "" ? `&select_shop_ids=${select_shop_ids}` : ""}${payment_paid_status !== "" ? `&payment_paid_status=${payment_paid_status}` : ""}${_status !== "" ? `&status=${_status}` : ""}&export_format=xlsx`)
    ...
    const configSearch = {
        search: [
            ...
            },
            {
                ...
                type: "RangePicker",
                name: "debt_due_date",
                label: GetIntlMessages("วันที่ชำระหนี้"),
                allowClear: true
            },
            {
                index: 1,
                ...
                allowClear: true,
                mode: "multiple",
                list: paymentStatus.length > 0 ? paymentStatus : [],
            },
            {
                index: 1,
                type: "select",
...
export default ReportCustomerDept
