    ...
    */
    ...
    const setColumnsTable = () => {
        const _column = [
            ...
            },
            {
                ...
                render: (text, record, index) => {
                    try {
                        if (record.ShopBusinessCustomer !== null) {
                            return record.ShopBusinessCustomer.customer_name
                        }
                        if (record.ShopPersonalCustomer !== null) {
            ...
            },
            {
                ...
                render: (text, record, index) => {
                    try {
                        if (record.ShopBusinessCustomer !== null) {
                            return record.ShopBusinessCustomer.tax_id
                        }
                        if (record.ShopPersonalCustomer !== null) {
                            return record.ShopPersonalCustomer.id_card_number
                        }
                    } catch (error) {
                        return ""
                    }
    ...
    const getDataSearch = async ({ search = modelSearch.search ?? "", limit = configTable.limit, page = configTable.page, sort = configSort.sort, order = (configSort.order === "descend" ? "desc" : "asc"), doc_date = modelSearch.doc_date, select_shop_ids = modelSearch.select_shop_ids ?? [], filter_zero = modelSearch.filter_zero ?? false }) => {
        try {
            ...
            let url = `/shopReports/salesTax?bolFilter_show_zero_vat=${filter_zero}&report_tax_type=sales_tax&arrStrFilter__status=0,1,2,3,4&arrStrFilter__payment_paid_status=${paymentStatusValue}&limit=${limit}&page=${page}&sort=${sort}&order=${order}&search=${search}${doc_date_startDate !== "" ? `&doc_date_startDate=${doc_date_startDate}` : ""}${doc_date_endDate !== "" ? `&doc_date_endDate=${doc_date_endDate}` : ""}${select_shop_ids !== "" ? `&select_shop_ids=${select_shop_ids}` : ""}`
    ...
    const exportExcel = async () => {
        try {
            ...
            const res = await API.get(`/shopReports/salesTax?bolFilter_show_zero_vat=${filter_zero}&report_tax_type=sales_tax&sort=doc_date&arrStrFilter__payment_paid_status=${paymentStatusValue}&limit=999999&page=1&search=${search}${doc_date_startDate !== "" ? `&doc_date_startDate=${doc_date_startDate}` : ""}${doc_date_endDate !== "" ? `&doc_date_endDate=${doc_date_endDate}` : ""}${select_shop_ids !== "" ? `&select_shop_ids=${select_shop_ids}` : ""}&export_format=xlsx`)
