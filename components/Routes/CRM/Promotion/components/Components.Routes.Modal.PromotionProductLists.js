...
import ProductWarehouse from "../components/Components.Routes.Modal.ProductWareHouse"

const { TextArea } = Input;

const ComponentsRoutesModalProductPromotionLists = ({ onFinish, calculateResult, mode }) => {
    ...
    const columns = [
        ...
        {
            title: () => GetIntlMessages("เปลี่ยนคลัง"),
            dataIndex: '',
            key: '',
            width: "5%",
            align: "center",
            render: (text, record, index) => {
                return (
                    <Button onClick={() => handleOpenInventoryBalanceModal(index, "2", record.warehouse_detail)} style={{ width: "100%" }} disabled={mode === "view" || !record.product_id}>เลือก</Button>
                )
            }
        },
        {
            ...
            <Fieldset style={{ marginTop: "20px" }} legend={`สรุปรายการ`}>
                <Row gutter={[10, 10]}>
                    <Col lg={12} md={12} xs={24}>
                        <Row gutter={[20, 10]}>
                            <Col span={12}>
                                <Form.Item label={GetIntlMessages("รายละเอียดโปรโมชั่น")} name="description">
...
export default ComponentsRoutesModalProductPromotionLists