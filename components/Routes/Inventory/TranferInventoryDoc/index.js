...
import { get, isArray, isEmpty, isPlainObject, isFunction, unionBy } from 'lodash'
...
import FormTranferInventoryDoc from './components/Components.Routes.Modal.FormTranferInventoryDoc'
import TranferInventoryDocLists from './components/Components.Routes.Modal.TranferInventoryDocLists'
import PrintOut from '../../../shares/PrintOut'

const { Text, Link } = Typography;

const TranferInventoryDoc = ({ docTypeId, title = null, callBack, filter_shop_id = null }) => {
    ...
    */
    const init = {
        ...
        modelSearch: {
            search: "",
            status: "default",
            shop_id: filter_shop_id !== null ? filter_shop_id : null,
            // select_shop_ids: "all"
        },
    }
    ...
    const onReset = () => {
        ...
        getDataSearch({
            ...
            shop_id: init.modelSearch.shop_id,
    ...
    * */
    const configSearch = {
        search: [
            ...
            },
            {
                ...
                type: "select",
                name: "status",
                label: GetIntlMessages("สถานะใบโอนสินค้า"),
            ...
            },
            {
                ...
                label: "เลือกสาขาต้นทาง",
                placeholder: "เลือกสาขาต้นทาง",
                allowClear: true,
                showSearch: true,
                // mode: "multiple",
                use: isFunction(callBack),
                list: [
                    // {
                    //     key: `ทุกสาขา`,
                    //     value: "all"
                    // },
                    ...isArray(shopInCorporate) && shopInCorporate?.length > 0 ? shopInCorporate.filter(x => x.id !== authUser?.UsersProfile?.ShopsProfile?.id).sort((a, b) => a.shop_config.shop_order_number - b.shop_config.shop_order_number)?.map(e => ({
                        key: !!e?.shop_name?.shop_local_name ? `${e?.shop_name?.[locale.locale]} (${e?.shop_name?.shop_local_name})` : e?.shop_name?.[locale.locale],
                        value: e?.id
                    })) : []],
            },
        ],
    ...
    const setColumnsTable = (searchStatus) => {
        ...
        _column.push(
            ...
            },
            {
                ...
                render: (text, record) => {
                    if (isFunction(callBack)) {
                        if (record.details.transfer_status_sucess) {
                            return (
                                <Text>{text}</Text>
                            )
                        } else {
            ...
            {
                title: () => GetIntlMessages("สาขาปลายทาง"),
                dataIndex: '',
                key: '',
                width: 250,
                render: (text, record) => {
                    let find = shopInCorporate?.find(x => x?.id === record?.details?.destination_branch)
                    if (find) {
            ...
            {
                title: () => GetIntlMessages("สถานะรับสินค้า"),
                dataIndex: 'details',
                key: 'details',
                width: 120,
                align: "center",
                render: (text, record) => !!text ? <div style={{ textAlign: "center", color: text.transfer_status_sucess ? "green" : "red" }}>{text.transfer_status_sucess ? "รับแล้ว" : "ยังไม่ได้รับ" ?? "-"}</div> : "-",
            },
            {
            ...
            },
            {
                ...
                render: (text, record) => (
                    <Button onClick={() => callBack(record)} disabled={record.details.transfer_status_sucess}>เลือก</Button>
                ),
            },
            {
    ...
    /* ค้นหา */
    const getDataSearch = async ({ search = modelSearch.search ?? "", limit = configTable.limit, page = configTable.page, sort = configSort.sort, order = (configSort.order === "descend" ? "desc" : "asc"), _status = modelSearch?.status ?? "default", shop_id = filter_shop_id ?? modelSearch.shop_id ?? "", select_shop_ids = modelSearch.select_shop_ids ?? [], }) => {
        try {
            if (page === 1) setLoading(true)
            // console.log("select_shop_ids", select_shop_ids)
            if (isArray(select_shop_ids)) {
                if (select_shop_ids.length > 1) {
                    let allPosition = select_shop_ids.indexOf('all')
                    if (allPosition !== -1) {
                        // console.log("allPosition", allPosition)
                        // console.log("select_shop_ids", select_shop_ids)
                        select_shop_ids.splice(allPosition, 1)
                    }
                }
            }
            setModelSearch({
            ...
            const res = await API.get(`/shopInventoryTransaction/all?search=${search}&status=${_status}&page=${page}&limit=${limit}&sort=${sort}&order=${order}${shop_id ? `&select_destination_id=${shop_id}` : ""}${docTypeId ? `&doc_type_id=${docTypeId}` : ""}${select_shop_ids ? `&select_shop_ids=${select_shop_ids}` : ""}`)

            if (res.data.status === "success") {
                const { currentCount, currentPage, pages, totalCount, data } = res.data.data;
                data = data.map(e => {
                    e.___update = e.status === 0 ? false : e.details.transfer_status_sucess ? false : true
                    e.___delete = false
                    e.disabled_change_status = e.status === 0 ? true : e.details.transfer_status_sucess
        ...
        try {
            ...
            if (id) {
                ...
                form.setFieldsValue({
                    ...
                    isModalVisible: true,
                    shop_sender_id: authUser?.UsersProfile?.ShopsProfile?.id,
                    shop_sender_name: !!authUser?.UsersProfile?.ShopsProfile?.shop_name.shop_local_name ? `${authUser?.UsersProfile?.ShopsProfile?.shop_name[locale.locale]} (${authUser?.UsersProfile?.ShopsProfile?.shop_name.shop_local_name})` : authUser?.UsersProfile?.ShopsProfile?.shop_name[locale.locale],
                    approver_id: authUser?.id,
                    approver_name: authUser?.UsersProfile?.fname[locale.locale] + " " + authUser?.UsersProfile?.lname[locale.locale],
                })
            }
            // setActiveKeyTab("1")
    ...
    const setFormValueData = async (value) => {
        try {
            ...
            let user = await getUserById(details?.approved_by)
            const model = {
                id: id,
                code_id: code_id,
                ...
                shop_recipient_id: details.destination_branch ?? null,
                shop_sender_id: shop_id ?? null,
                shop_sender_name: !!ShopsProfiles.shop_name.shop_local_name ? `${ShopsProfiles.shop_name[locale.locale]} (${ShopsProfiles.shop_name.shop_local_name})` : ShopsProfiles.shop_name[locale.locale],
    ...
    return (
        <>
            ...
            <ModalFullScreen
                ...
                title={<ModalFullScreenTitle title={`ใบโอนระหว่างสาขา` ?? documentTypesName} isShowTittle={true} />}
                loading={loading}
                CustomsButton={() => {
                    return (
                                        ...
                                        items={[
                                            {
                                                ...
                                                key: '1',
                                                children: <TranferInventoryDocLists calculateResult={calculateResult} mode={configModal.mode} />,
                                            },
                                        ]}
                                    />
                                </div>
...
export default TranferInventoryDoc