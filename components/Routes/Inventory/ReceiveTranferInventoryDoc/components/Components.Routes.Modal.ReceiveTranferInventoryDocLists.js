import { Input, Table, Button, Row, Col, Form, Select, Popconfirm, InputNumber, Popover, Badge, Modal, Tabs, Tooltip } from "antd";
...
import ShopWarehouse from "../../../../../routes/MyData/Setting/WarehousesDatasManagement";
import ShopShelf from "../components/Components.Routes.Modal.ShopShelf"

const { TextArea } = Input;

const ReceiveTranferInventoryDocLists = ({ onFinish, calculateResult, mode }) => {
    ...
    const [isShopWarehouseModalVisible, setIsShopWarehouseModalVisible] = useState(false);
    const [isShopWarehouseAllModalVisible, setIsShopWarehouseAllModalVisible] = useState(false);
    const [listIndex, setListIndex] = useState(0);
    const [activeKeyTab, setActiveKeyTab] = useState("1");
    const [activeKeyTabAll, setActiveKeyTabAll] = useState("1");
    ...
    const columns = [
        ...
        {
            title: () => <><span style={{ color: "red" }}>* </span>{GetIntlMessages("คลัง")}</>,
            dataIndex: '',
            key: '',
            align: "center",
            render: (text, record, index) => {
                return (
                    <Form.Item
                        ...
                        style={{ margin: 0 }}
                        name={['product_list', index, "warehouse_name"]}>
                        <TextArea autoSize disabled />
                    </Form.Item>
                )
            }
        ...
        {
            title: () => <><span style={{ color: "red" }}>* </span>{GetIntlMessages("ชั้น")}</>,
            dataIndex: '',
            key: '',
            align: "center",
            render: (text, record, index) => {
                return (
                    <Form.Item
                        ...
                        style={{ margin: 0 }}
                        name={['product_list', index, "shelf_name"]}>
                        <TextArea autoSize disabled />
                    </Form.Item>
                )
            }
        ...
        {
            title: () => (
                <Tooltip title="เลือกคลังและชั้น ทุกแถว">
                    <Button onClick={() => handleOpenShopWarehouseAllModal()} style={{ width: "100%" }} disabled={mode === "view"}>
                        {GetIntlMessages("เลือก")}
                    </Button>
                </Tooltip>
            ),
            dataIndex: '',
            ...
            render: (text, record, index) => {
                return (
                    <Button onClick={() => handleOpenShopWarehouseModal(index, record.shelf_list)} style={{ width: "100%" }} disabled={mode === "view"}>เลือก</Button>
                )
            }
        },
        {
            title: () => (GetIntlMessages("ต้นทุนต่อหน่วย")),
            dataIndex: '',
            key: '',
            width: "10%",
            align: "center",
            ...
            render: (text, record, index) => (
                <>
                    <Form.Item rules={[
                        ...
                        <InputNumber min={1} disabled stringMode style={{ width: "100%" }} formatter={(value) => !!value && value.length > 0 ? `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',') : ""}
    ...
    const handleCancelShopWarehouseModal = () => {
        try {
            setIsShopWarehouseModalVisible(false)
        } catch (error) {
            console.log("handleCancelShopWarehouseModal error: ", error)
        }
    }

    const handleOpenShopWarehouseModal = async (index, list) => {
        try {
            console.log("list", list)
            const { product_list } = form.getFieldValue();
            setListIndex(index)
            if (!!product_list[index].warehouse_id) {
                setActiveKeyTab("2")
                setListData(list)
            } else {
                setActiveKeyTab("1")
            }
            setIsShopWarehouseModalVisible(true)


        } catch (error) {
            console.log("handleOpenShopWarehouseModal error: ", error)
        }
    }

    const handleOpenShopWarehouseAllModal = async () => {
        setActiveKeyTab("1")
        setIsShopWarehouseAllModalVisible(true)
    }

    const callbackShopWarehouse = (data, indexCallBack) => {
        try {
            const { product_list } = form.getFieldValue();
            setListData(data?.shelf)
            product_list[indexCallBack].warehouse_id = data?.id
            product_list[indexCallBack].warehouse_name = data?.name[locale.locale]
            product_list[indexCallBack].shelf_list = data?.shelf

            form.setFieldsValue({ product_list })
            setActiveKeyTab("2")
            calculateTable()
        } catch (error) {
            console.log("callbackShopWarehouse : ", error)
        }
    }

    const handleCancelShopWarehouseAllModal = () => {
        try {
            setIsShopWarehouseAllModalVisible(false)
        } catch (error) {
            console.log("handleCancelShopWarehouseAllModal error: ", error)
        }
    }

    const callbackShopWarehouseAll = (data, indexCallBack) => {
        try {

            const { product_list } = form.getFieldValue();
            setListDataAll(data?.shelf)
            console.log("indexCallBack", product_list.length)
            product_list.map((e, i) => {
                e.warehouse_id = data?.id
                e.warehouse_name = data?.name[locale.locale]
                e.shelf_list = data?.shelf
            })
            // product_list[indexCallBack].warehouse_id = data?.id
            // product_list[indexCallBack].warehouse_name = data?.name[locale.locale]
            // product_list[indexCallBack].shelf_list = data?.shelf
            ...
            form.setFieldsValue({ product_list })
            setActiveKeyTabAll("2")
            calculateTable()
        } catch (error) {
            console.log("callbackShopWarehouseAll : ", error)
        }
    }

    const callbackShopShelfAll = (data, indexCallBack) => {
        try {
            const { product_list } = form.getFieldValue();
            product_list.map((e, i) => {
                e.shelf_code = data.code
                e.shelf_name = data.name[locale.locale]
            })
            form.setFieldsValue({ product_list })
            setIsShopWarehouseAllModalVisible(false)
            calculateTable()
            ...
            console.log("callbackShopShelfAll : ", error)
        }
    }

    const callbackShopShelf = (data, indexCallBack) => {
        try {
            const { product_list } = form.getFieldValue();

            product_list[indexCallBack].shelf_code = data.code
            product_list[indexCallBack].shelf_name = data.name[locale.locale]
            form.setFieldsValue({ product_list })
            setIsShopWarehouseModalVisible(false)
            calculateTable()
            form.validateFields()
    ...
    const handleChangeTabsAll = (key) => {
        try {
            setActiveKeyTabAll(() => key)
        } catch (error) {
    ...
    return (
        <>
            ...
            <Modal
                maskClosable={false}
                open={isShopWarehouseModalVisible}
                onCancel={handleCancelShopWarehouseModal}
                width="90vw"
                style={{ top: 16 }}
                footer={(
                    <>
                        <Button onClick={() => handleCancelShopWarehouseModal()}>{GetIntlMessages("กลับ")}</Button>
                    </>
                )}
            >
                <Tabs
                    ...
                    items={[
                        {
                            ...
                            key: '1',
                            children: <ShopWarehouse title={"เลือกคลังสินค้ารายการที่ " + (listIndex + 1)} callBack={callbackShopWarehouse} listIndex={listIndex} />,
                        },
                        {
                            label: (<span>ชั้นสินค้า</span>),
                            key: '2',
                            disabled: true,
                            children: <ShopShelf title={"เลือกชั้นสินค้ารายการที่ " + (listIndex + 1)} callBack={callbackShopShelf} listIndex={listIndex} listData={listData} />,
                        },
                    ]}
                />
            </Modal>
            ...
            <Modal
                maskClosable={false}
                open={isShopWarehouseAllModalVisible}
                onCancel={handleCancelShopWarehouseAllModal}
                width="90vw"
                style={{ top: 16 }}
                footer={(
                    <>
                        <Button onClick={() => handleCancelShopWarehouseAllModal()}>{GetIntlMessages("กลับ")}</Button>
                    </>
                )}
            >
                <Tabs
                    defaultActiveKey="1"
                    activeKey={activeKeyTabAll}
                    onChange={handleChangeTabsAll}
                    items={[
                        {
                            label: (<span>คลังสินค้า</span>),
                            key: '1',
                            children: <ShopWarehouse title={"เลือกคลังสินค้ารายการที่ "} callBack={callbackShopWarehouseAll} />,
                        },
                        {
                            label: (<span>ชั้นสินค้า</span>),
                            key: '2',
                            disabled: true,
                            children: <ShopShelf title={"เลือกชั้นสินค้ารายการที่ "} callBack={callbackShopShelfAll} listData={listDataAll} />,
                        },
                    ]}
                />
            </Modal>
...
export default ReceiveTranferInventoryDocLists