...
const PaymentDocsV2 = ({ docId, title, loading, handleCancelDebtDoc, initForm, carPreLoading, setCarPreLoading, fromTable = false, debtDocObj = null }) => {
        ...
        try {
            ...
            _column.push(
                ...
                },
                {
                    ...
                    width: "5%",
                    align: "center",
                    render: (text, record) => get(record, `code_id`, record?.ShopInventoryTransaction?.code_id ?? record?.ShopPartnerDebtCreditNoteDoc?.code_id ?? record?.ShopPartnerDebtDebitNoteDoc?.code_id)
                },
                {
    ...
    const extractDataDocSaleType = (record, type) => {
        try {
            ...
            } else if (record.ShopPartnerDebtDebitNoteDoc) {
                return RoundingNumber(Number(record.price_grand_total)) ?? '-'
            } else {
    ...
    const handleVisibleModal = async () => {
        try {
            ...
            const { data } = await API.get(`/shopPartnerDebtDoc/byId/${docId}`), { ShopPartnerDebtLists, ShopPaymentTransactions } = data.data
            if (data.status === "success") {
                ...
                setDataSource(() => [...ShopPartnerDebtLists])
                                ...
                                <div className="pt-3 pb-2">
                                    ...
                                    <div className="p-3">
                                        <Row gutter={[20, 10]}>
                                            ...
                                            <Col xl={6} lg={12} md={6} sm={12} xs={12} hidden>
                                                <div>
                                                    <Cheque
                                                        icon={
