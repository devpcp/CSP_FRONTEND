    ...
    * */
    const configSearch = {
        ...
        button: {
            create: true,
            name: {
                add: GetIntlMessages(`สร้าง${`ใบลดหนี้ลูกหนี้` ?? documentTypesName}`),
            },
            download: false,
            import: false,
            export: false,
    ...
    const getDataSearch = async ({ search = modelSearch.search ?? "", limit = configTable.limit, page = configTable.page, sort = configSort.sort, order = (configSort.order === "descend" ? "desc" : "asc"), _status = modelSearch?.status ?? "active" }) => {
        try {
            ...
            const res = await API.get(`/shopCustomerDebtCreditNoteDoc/all?search=${search}&status=${_status}&page=${page}&limit=${limit}&sort=${sort}&order=${order}`)
        ...
        try {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    setLoading(true)
                    const { data } = await API.put(`/shopCustomerDebtCreditNoteDoc/put/${id}`, { status })
    ...
    const addEditViewModal = async (mode, id, isPayment, isInovices) => {
        try {
            ...
            if (id) {
                const { data } = await API.get(`/shopCustomerDebtCreditNoteDoc/byId/${id}`)
    ...
    const setFormValueData = async (value) => {
        try {
            ...
            const { ShopCustomerDebtCreditLists, details, bus_customer_id, per_customer_id, ShopPersonalCustomer, ShopBusinessCustomer, shop_temporary_delivery_order_doc_id, debt_credit_note_type } = value, customer_type = !!per_customer_id ? "person" : "business";
            ...
            const model = {
                ...
                arr_debt_list: ShopCustomerDebtCreditLists,
    ...
    const onFinish = async (values) => {
        try {
            ...
            if (configModal.mode === "add") {
                ...
                const { id } = form.getFieldValue()
                res = await API.put(`/shopCustomerDebtCreditNoteDoc/put/${id}`, model)
            }
    ...
    return (
        <>
            ...
            <ModalFullScreen
                ...
                title={<ModalFullScreenTitle title={`ใบลดหนี้ลูกหนี้` ?? documentTypesName} isShowTittle={true} />}
                loading={loading}
                CustomsButton={() => {
                    return (
