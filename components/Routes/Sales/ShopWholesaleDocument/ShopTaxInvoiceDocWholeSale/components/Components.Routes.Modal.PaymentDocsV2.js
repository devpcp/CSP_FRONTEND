import { Button, Col, Divider, Form, Row, Table, Dropdown, Space, Checkbox } from 'antd'
import React from 'react'
import GetIntlMessages from '../../../../../../util/GetIntlMessages'
import ModalFullScreen from '../../../../../shares/ModalFullScreen'
import { useState } from 'react'
import { DollarCircleOutlined, FileExcelOutlined, DownOutlined, BankOutlined, DollarOutlined } from '@ant-design/icons'
import API from '../../../../../../util/Api'
import { useEffect } from 'react'
import { get, isEmpty, isFunction, isPlainObject, isArray } from 'lodash'
...
import Debtor from '../../../Invoices/Payment/Components.Routes.Modal.PayWith.Debtor.js';
import Swal from 'sweetalert2'
import CarPreloader from '../../../../../_App/CarPreloader'
import moment from 'moment'

const PaymentDocsV2 = ({ docId, title, loading, handleCancelTaxDoc, initForm, setCarPreLoading, fromTable = false, serviceOrderDocObj = null }) => {
    ...
    const handleVisibleModal = async () => {
        try {
            ...
            const { data } = await API.get(`/shopTaxInvoiceDoc/byId/${docId}`), { ShopPaymentTransactions } = isPlainObject(serviceOrderDocObj) ? serviceOrderDocObj : initForm.getFieldValue("ShopServiceOrderDoc")
            // console.log('data :>> ', data);
            if (data.status === "success") {
                const list_service_product = data.data.ShopTaxInvoiceLists, model = data.data, payment_method_list = [...ShopPaymentTransactions]
                ...
                // let res
                // res =  await API.get(`/shopPaymentTransaction/all?shop_service_order_doc_id=${shop_service_order_doc_id}&shop_tax_invoice_doc_id=${id}&filterShowOnlyNonCanceledPayment=false&page=1&limit=999999&sort=created_date&order=desc`)
                // console.log('res :>> ', res);
                // if(res.data.status === "success"){
                //     const list_service_product = data.data.ShopTaxInvoiceLists, model = data.data , payment_method_list = [...res.data.data.data]
    ...
    const callbackPayment = async (type, value, isPartialPayment) => {
        try {
            ...
            // console.log('value callbackPayment tax:>> ', value);
            const { shop_id } = authUser.UsersProfile, { shop_service_order_doc_id, doc_date, id, is_abb, is_inv, status } = form.getFieldValue()
            ...
            if (res.data.status === "success") {
                ...
                if (status === 1) {
                    if (is_abb) delete inputOptions.is_abb
                    if (is_inv) delete inputOptions.is_inv
                }
                ...
                if (isEmpty(inputOptions)) {
                    ...
                    handleCancel()
                    if (isFunction(handleCancelTaxDoc)) handleCancelTaxDoc()
                } else {
                    Swal.fire({
                        ...
                        input: "select",
                        inputOptions,
                        // confirmButtonText: GetIntlMessages("สร้างใบกำกับภาษีอย่างย่อ"),
                        ...
                        if (result.isConfirmed) {
                            ...
                            resTax = await API.put(`/shopTaxInvoiceDoc/put/${id}`, DeliveryOrderModel)
                        ...
                        >
                            <Row gutter={[30]}>
                                ...
                                <Col lg={12} md={24} xs={24}>
                                                        ...
                                                        <PartialPayment
                                                            icon={(
                                                                <div style={{ width: "100%", height: "100%" }}>
                                                                    <img style={{ width: "30%", marginTop: 20, marginBottom: 0 }} src='/assets/images/icon/cash.svg' />
                                                                    <Divider style={{ marginTop: "20%", marginBottom: 0, height: "2rem", paddingTop: "40%" }} type='vertical' />
                                                                    <img style={{ width: "35%", paddingTop: 20, margin: 0 }} src='/assets/images/icon/credit-card.svg' />
                                                                    <Divider style={{ padding: 0, marginTop: 10, marginBottom: 0 }} />
                                                                    <img style={{ width: "40%", padding: 0, margin: 0 }} src='/assets/images/icon/money-transfer.svg' />
                                                                </div>
                                                            )}
                                    ...
                                    <div className="pt-3 pb-2">
                                        <div className="border-bottom pb-2"><h1>รายการส่วนลด</h1></div>
                                        <div className="p-3 flex-container-herder">
                                            <div className="pr-5">
                                                <ComponentsDiscount
