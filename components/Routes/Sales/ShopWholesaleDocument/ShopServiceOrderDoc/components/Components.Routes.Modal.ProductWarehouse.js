...
const ComponentsRoutesModalProductWarehouse = ({ title = null, callBack, listData, listIndex, }) => {
    ...
    const getMasterData = async () => {
        try {
            ...
            setListDataTable(newList.filter(x => x.shelf.balance !== "0").sort((a, b) => a?.shelf?.dot_mfd === undefined || b?.shelf?.dot_mfd === undefined ? -1 : Number(a?.shelf?.dot_mfd.slice(-2)) - Number(b?.shelf?.dot_mfd.slice(-2))))
    ...
    const setColumnsTable = () => {
        const _column = [
            ...
            },
            {
                ...
                width: 300,
                use: true,
                render: (text, record) => get(text, `Shelf.name.${locale.locale}`, "-"),
            },
            {
                title: "DOT",
                dataIndex: "shelf",
                ...
                width: 150,
                use: true,
                render: (text, record) => get(text, `dot_mfd`, "-"),
            },
            {
                title: "จำนวน",
                dataIndex: "shelf",
                ...
                width: 150,
                use: true,
                render: (text, record) => get(text, `balance`, "-"),
            },
            {
                title: "หน่วย",
                dataIndex: "shelf",
                ...
                width: 150,
                use: true,
                render: (text, record) => get(text, `PurchaseUnit.type_name.${locale.locale}`, "-"),
            },
            {
...
export default ComponentsRoutesModalProductWarehouse;
