...
import FormDebtorDoc from './components/Components.Routes.Modal.FormDebtorDoc'
import DocumentDebtLists from './components/Components.Routes.Modal.Tab1.DocumentDebtLists'


const ShopPartnerDebtBillAcceptanceDoc = ({ docTypeId, title = null, callBack }) => {
    ...
    * */
    const configSearch = {
        ...
        button: {
            create: true,
            name: {
                add: GetIntlMessages(`สร้าง${`ใบรับวางบิลเจ้าหนี้` ?? documentTypesName}`),
            },
            download: false,
            import: false,
            export: false,
    ...
    const setFormValueData = (value) => {
        try {
            const { ShopCustomerDebtBillingNoteLists, details, bus_customer_id, per_customer_id, ShopPersonalCustomer, ShopBusinessCustomer } = value, customer_type = !!per_customer_id ? "person" : "bussiness";
    ...
    const onFinish = async (values) => {
        try {
            ...
            if (res.data.status === "success") {
                Swal.fire({
                    title: GetIntlMessages("บันทึกใบรับวางบิลเจ้าหนี้สำเร็จ !!"),
                    icon: "success",
                    timer: 2000
                })
                handleCancel()
    ...
    return (
        <>
            ...
            <ModalFullScreen
                ...
                title={<ModalFullScreenTitle title={`ใบรับวางบิลเจ้าหนี้` ?? documentTypesName} isShowTittle={true} />}
                loading={loading}
                CustomsButton={() => {
                    return (
                        ...
                        carPreLoading ? <CarPreloader />
                            ...
                            <div className="container-fluid">
                                <div className='pr-5 pl-5 detail-before-table'>
                                    ...
                                    <FormDebtorDoc mode={configModal.mode} calculateResult={calculateResult} disabledWhenDeliveryDocActive={disabledWhenDeliveryDocActive} />
                                </div>
                                        ...
                                        items={[
                                            {
                                                ...
                                                key: '1',
                                                children: <DocumentDebtLists calculateResult={calculateResult} mode={configModal.mode} disabledWhenDeliveryDocActive={disabledWhenDeliveryDocActive} />,
                                            },
                                        ]}
                                    />
...
export default ShopPartnerDebtBillAcceptanceDoc