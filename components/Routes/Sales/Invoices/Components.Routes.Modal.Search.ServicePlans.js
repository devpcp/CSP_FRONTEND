 ...
 * @returns 
 */
const ComponentsRoutesModalSearchServicePlans = ({ mode, form, callback }) => {
    ...
    */
    ...
    /* ค้นหา */
    const getDataSearch = async ({ search = modelSearch.search ?? "", limit = configTable.limit, page = configTable.page, sort = configSort.sort, order = (configSort.order === "descend" ? "desc" : "asc"), _status = 2 }) => {
        try {
            if (page === 1) setLoading(true)
            const res = await API.get(`/shopSalesTransactionDoc/all?limit=${limit}&page=${page}&sort=${sort}&order=${order}&status=${_status}&search=${search}&doc_type_id=7ef3840f-3d7f-43de-89ea-dce215703c16`)
...
export default ComponentsRoutesModalSearchServicePlans