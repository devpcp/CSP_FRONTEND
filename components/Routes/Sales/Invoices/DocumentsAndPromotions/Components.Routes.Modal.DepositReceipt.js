...
const DepositReceipt = ({ icon, textButton, disabled }) => {
    ...
    const setColumnsTable = () => {
        const _column = [
            ...
            {
                title: () => GetIntlMessages("เลบที่ใบรับมัดจำ"),
                dataIndex: 'code_id',
                key: 'code_id',
                align: "center",
                width: 100,
                render: (text, record, index) => record ? <Button type='link' onClick={() => selectedData(record)}>{text}</Button> : "-",
            },
            {
                title: GetIntlMessages("วันที่มัดจำ"),
                dataIndex: 'code_id',
                key: 'code_id',
                width: 150,
                align: "center",
                // render: (text, record) => console.log('text', text),
                // render: (text, record) => record ?<Button type='link' onClick={()=>selectedData(record)}>{text}</Button>  : "-",
        ...
        <>
            <>
                ...
                <ModalFullScreen
                    width={750}
                    maskClosable={false}
                    title={`${GetIntlMessages("รายการใบรับเงินมัดจํา")}`}
