    ...
    * */
    ...
    const getDataSearch = async ({ search = modelSearch.search ?? "", limit = configTable.limit, page = configTable.page, sort = configSort.sort, order = (configSort.order === "descend" ? "desc" : "asc"), _status = modelSearch?.status ?? "active" }) => {
        try {
            ...
            const res = await API.get(`/shopPartnerDebtDebitNoteDoc/all?search=${search}&status=${_status}&page=${page}&limit=${limit}&sort=${sort}&order=${order}`)
        ...
        try {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    setLoading(true)
                    const { data } = await API.put(`/shopPartnerDebtDebitNoteDoc/put/${id}`, { status })
    ...
    const addEditViewModal = async (mode, id, isPayment, isInovices) => {
        try {
            ...
            if (id) {
                const { data } = await API.get(`/shopPartnerDebtDebitNoteDoc/byId/${id}`)
    ...
    const setFormValueData = async (value) => {
        try {
            ...
            const { ShopPartnerDebtDebitNoteLists, details, bus_partner_id, ShopBusinessPartner, shop_inventory_import_doc_id } = value;
            ...
            const model = {
                ...
                options_list,
                arr_debt_list: ShopPartnerDebtDebitNoteLists,
    ...
    const onFinish = async (values) => {
        try {
            ...
            const model = {
                ...
                shopPartnerDebtDebitNoteLists: !!arr_debt_list && arr_debt_list.length > 0 ?
            ...
            if (configModal.mode === "add") {
                res = await API.post(`/shopPartnerDebtDebitNoteDoc/add`, model)
            } else if (configModal.mode === "edit") {
                const { id } = form.getFieldValue()
                res = await API.put(`/shopPartnerDebtDebitNoteDoc/put/${id}`, model)
            }

            if (res.data.status === "success") {
                Swal.fire({
                    title: GetIntlMessages("บันทึกใบเพิ่มหนี้เจ้าหนี้สำเร็จ !!"),
                    icon: "success",
                    timer: 2000
                })
                handleCancel()
    ...
    return (
        <>
            ...
            <ModalFullScreen
                ...
                title={<ModalFullScreenTitle title={`ใบเพิ่มหนี้เจ้าหนี้` ?? documentTypesName} isShowTittle={true} />}
                loading={loading}
                CustomsButton={() => {
                    return (
