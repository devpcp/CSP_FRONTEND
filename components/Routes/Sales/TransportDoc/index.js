...
import FormTemporaryDeliveryOrderDoc from './components/Components.Routes.Modal.FormTransportDoc'
import Tab1ServiceAndProduct from './components/Components.Routes.Modal.Tab1.DocumentLists'


const TransportDoc = ({ docTypeId }) => {
    ...
    * */
    const configSearch = {
        search: [
            ...
            },
            {
                ...
                type: "select",
                name: "status",
                label: GetIntlMessages("สถานะใบขนส่งสินค้า"),
    ...
    const getDataSearch = async ({ search = modelSearch.search ?? "", limit = configTable.limit, page = configTable.page, sort = configSort.sort, order = (configSort.order === "descend" ? "desc" : "asc"), _status = modelSearch?.status ?? "active" }) => {
        try {
            ...
            const res = await API.get(`/shopCustomerDebtBillingNoteDocs/all?filter__debt_price_amount_left=false&search=${search}&status=${_status}&page=${page}&limit=${limit}&sort=${sort}&order=${order}`)
    ...
    const setFormValueData = (value) => {
        try {
            ...
            form.setFieldsValue({
                ...
                customer_type,
                employee_list: customer_type === "person" ? customCustomerName([ShopPersonalCustomer], customer_type) : customCustomerName([ShopBusinessCustomer], customer_type),
    ...
    const onFinish = async (values) => {
        try {
            ...
            // if (res.data.status === "success") {
            //     Swal.fire({
            //         title: GetIntlMessages("บันทึกใบขนส่งสินค้า !!"),
            //         icon: "success",
            //         timer: 2000
            //     })
            //     handleCancel()
    ...
    return (
        <>
            ...
            <ModalFullScreen
                ...
                title={<ModalFullScreenTitle title={`ใบขนส่งสินค้า` ?? documentTypesName} isShowTittle={true} />}
                loading={loading}
                CustomsButton={() => {
                    return (
                                        ...
                                        items={[
                                            {
                                                ...
                                                key: '1',
                                                children: <Tab1ServiceAndProduct calculateResult={calculateResult} mode={configModal.mode}  />,
                                            },
                                        ]}
                                    />
                                </div>
