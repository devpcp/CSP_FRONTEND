...
import FormRepairOrder from './components/Components.Routes.Modal.FormRepairOrder'
...
const RepairOrder = ({ docTypeId, view_doc_id, select_shop_ids, title = null, }) => {
    ...
    const configButtonPrintOutFromTable = {
        repair_order: {
            status: true, name: "ใบสั่งซ่อม", price_use: 1
        },
        withdraw_product: {
    ...
    * */
    ...
    const setColumnsTable = (searchStatus) => {
        ...
        _column.push(
            ...
            },
            {
                ...
                render: (text, record) => {
                    return (
                        <PrintOut textButton={"พิมพ์ใบสั่งซ่อม"} documentId={record?.id} style={{ width: "100%", display: "flex", justifyContent: "center", alignItems: "center", overflow: "hidden" }} morePrintOuts={configButtonPrintOutFromTable} customPriceUse={true} docTypeId={docTypeId} settingPrint={settingPrint} />
                    )
                },
            },
    ...
    const getDataSearch = async ({ search = modelSearch.search ?? "", limit = configTable.limit, page = configTable.page, sort = configSort.sort, order = (configSort.order === "descend" ? "desc" : "asc"), _status = "active", doc_date_startDate = isArray(modelSearch.select_date) ? modelSearch.select_date[0] ?? "" : null, doc_date_endDate = isArray(modelSearch.select_date) ? modelSearch.select_date[1] ?? "" : null, payment_paid_status = modelSearch.payment_paid_status }) => {
        try {
            ...
            const res = await API.get(`/shopServiceOrderDoc/all?search=${search}&status=${_status}&page=${page}&limit=${limit}&sort=${sort}&order=${order}&doc_sales_type=1${doc_date_startDate ? `&doc_date_startDate=${moment(doc_date_startDate).format("YYYY-MM-DD")}` : ""}${doc_date_endDate ? `&doc_date_endDate=${moment(doc_date_endDate).format("YYYY-MM-DD")}` : ""}${payment_paid_status ? `&payment_paid_status=${payment_paid_status}` : ""}${select_shop_ids ? `&select_shop_ids=${select_shop_ids}` : ""} `)
    ...
    const setFormValueData = (value) => {
        try {
            ...
            // console.log("ShopServiceOrderLists",ShopServiceOrderLists)
    ...
    const confirmOrder = async () => {
        try {
            ...
            // console.log('resutValidList :>> ', resutValidList);
            // const invalidList = []
            ...
            if (validStatus) {
                // if (invalidList.length === 0) {
                Swal.fire({
                    title: GetIntlMessages("ยืนยันใบสั่งซ่อมนี้หรือไม่ ?"),
                ...
                Swal.fire({
                    title: `รายการที่ ${errorArr.map(e => e.index + 1)} มีข้อมูล \n ${errorArr.map(e => e.descript)} \n ไม่ครบถ้วน กรุณาแก้ไขให้ถูกต้อง`,
    ...
    const createDeliveryOrderDoc = async () => {
        try {
            Swal.fire({
                    ...
                    if (data.status === "success") {
                        ...
                        Swal.fire({ title: 'ยืนยันรายการสำเร็จ !! ท่านต้องการไปยังหน้าใบส่งของหรือไม่ ?', icon: 'question', confirmButtonText: GetIntlMessages("submit"), confirmButtonColor: mainColor, showCancelButton: true, cancelButtonText: GetIntlMessages("cancel") }).then((resultVal) => {
                            if (resultVal.isConfirmed) {
                                router.push({
                                    pathname: "ShopTemporaryDeliveryOrderDoc",
                                    // query: { search: search }
                                })
                            }
                        })
                    ...
                    switch (modeKey) {
                        ...
                        case 1:
                            ...
                            if (validStatus) {
                                // if (invalidList.length === 0) {
                                Swal.fire({
                                    title: GetIntlMessages(`ยืนยันการ "ยืนยันใบสั่งซ่อม" นี้หรือไม่ ?`),
                                    text: GetIntlMessages("ใบสั่งซ่อมจะกลายเป็นฉบับจริง และเลขใบสั่งซ่อมจะมีการเปลี่ยนแปลง !!"),
                                    icon: "question",
                                    showCancelButton: true,
                                    cancelButtonText: GetIntlMessages("cancel"),
    ...
    const onFinish = async (values, isConfirmedDoc = false) => {
        try {
            ...
            // if (isArray(model.shopServiceOrderLists) && model.shopServiceOrderLists.length > 0) {
            //     model.shopServiceOrderLists.map((e, index) => {
            ...
            if (validStatus) {
                if (configModal.mode === "add") {
                    ...
                    const { id } = form.getFieldValue()
                    model.status = 1
                    res = await API.put(`/shopServiceOrderDoc/put/${id ?? idEdit}`, model)
                }
    ...
    const resultSubmit = (res) => {
        try {
            if (res.data.status === "success") {
                Swal.fire({
                    title: GetIntlMessages("บันทึกใบสั่งซ่อมสำเร็จ !!"),
                    icon: "success",
                    timer: 2000
                })
                handleCancel()
    ...
    const handleMenuClick = async (e) => {
        try {
            setLoading(true)
            const { id, ShopTaxInvoiceDocs } = form.getFieldValue(), findTaxInovicesActiveDoc = !!ShopTaxInvoiceDocs ? ShopTaxInvoiceDocs.find(where => where.shop_service_order_doc_id === id && where.status === 1) : [];
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    setCarPreLoading(true)
                    // res = await API.post(`/shopTaxInvoiceDoc/add`, model)
                    if ((isArray(ShopTaxInvoiceDocs) && ShopTaxInvoiceDocs.length === 0) || !findTaxInovicesActiveDoc || !ShopTaxInvoiceDocs) {
                        res = await API.post(`/shopTaxInvoiceDoc/add`, model)
                    } else {
    ...
    const configButtonPrintOut = {
        repair_order: {
            status: form.getFieldValue("status") == 1 && form.getFieldValue("is_draft") === false
                ? true
                : false, name: "ใบสั่งซ่อม", price_use: 1
        },
        withdraw_product: {
            status: form.getFieldValue("status") == 1 && form.getFieldValue("is_draft") === false
                ? true
        ...
        <>
                ...
                CustomsButton={() => {
                    return (
                        <div style={{ width: "100%", display: "flex", justifyContent: "end" }}>
                            ...
                            <div style={{ width: "20%" }} className='ml-2'>
                                ...
                                <Row gutter={[10, 10]} justify="end" style={{ width: "100%" }}>
                                    <>
                                        ...
                                        <Col xxl={4} lg={6} md={12} xs={24} hidden={configModal.mode === "view" || disabledWhenDeliveryDocActive || disabledWhenTaxInvoiceDocActive} >
                                            ...
                                            {/* <Dropdown overlay={()=> menuOnFinish()} style={{ width: "100%" }} loading={loading}>
                                                <Button type='primary' onClick={(e) => e.preventDefault()} style={{ width: "100%", padding: 4 }}>
                                                    <Space>
                                                        เลือกการบันทึก
                                                        <DownOutlined />
                                                    </Space>
                                        ...
                                        <Col xxl={4} lg={6} md={12} xs={24} hidden={configModal.mode === "add" || form.getFieldValue("status") != 1} >
                                            <PrintOut textButton={"พิมพ์ใบสั่งซ่อม"} loading={loading || carPreLoading} documentId={form.getFieldValue("id")} style={{ width: "100%", display: "flex", justifyContent: "center", alignItems: "center", overflow: "hidden" }} morePrintOuts={configButtonPrintOut} customPriceUse={true} docTypeId={docTypeId} />
                                        </Col>
                                        <Col xxl={4} lg={6} md={12} xs={24} hidden={configModal.mode !== "edit" || form.getFieldValue("is_draft") === false || form.getFieldValue("status") != 1} >
                                            <Button loading={loading || carPreLoading} onClick={() => confirmOrder()} type="text" style={{ width: "100%", borderColor: "black" }}>{GetIntlMessages("ยืนยันใบสั่งซ่อม")}</Button>
                                        </Col>
                                        <Col xxl={4} lg={6} md={12} xs={24} hidden={configModal.mode === "view" || form.getFieldValue("is_draft") !== false || disabledWhenDeliveryDocActive || form.getFieldValue("status") != 1}>
                                            <Button loading={loading || carPreLoading} onClick={() => createDeliveryOrderDoc()} type="text" style={{ overflow: "hidden", width: "100%", borderColor: "black", padding: 2 }}>{GetIntlMessages("สร้างใบส่งของชั่วคราว")}</Button>
                                        </Col>
            ...
            >
                ...
                >
                    ...
                    <Form.Item name="upload_car_list">
