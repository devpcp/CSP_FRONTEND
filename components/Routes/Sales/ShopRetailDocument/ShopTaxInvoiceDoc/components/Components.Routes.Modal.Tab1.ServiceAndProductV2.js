...
const ComponentsRoutesModalTab1ServiceAndProductV2 = ({ onFinish, calculateResult, mode }) => {
            ...
            },
            {
                ...
                render: (record, text, index) => (
                    <Form.Item key={`change-name-${index}`} style={{ margin: 0 }} name={[index, "change_name_status"]}>
                        <Checkbox checked={record?.change_name_status ?? false} disabled={true} onChange={(checkedValue) => handleCheckChangeName(checkedValue.target.checked, index, record)} />
                    </Form.Item>
                )
            },
            // {
                ...
                render: (text, record, index) => (
                    <>
                        <Form.Item style={{ margin: 0 }} name={[index, "price_unit"]}>
                            <InputNumber disabled={true} size="large" style={{ width: "100%" }} step="1" stringMode precision={2}
                        ...
                        render: (text, record, index) => (
                            <>
                                <Form.Item style={{ margin: 0 }} name={[index, "dot_mfd"]}>
                                    <Select disabled={true} size="large" showSearch optionFilterProp="children" filterOption={(inputValue, option) => (option?.children ?? '').includes(inputValue)}>
                                ...
                                <Row gutter={[0, 10]}>
                                    <Col span={24}>
                                        <Form.Item style={{ margin: 0 }} name={[index, "warehouse"]}>
                                            <Select disabled={true} size="large" showSearch placeholder="คลัง" filterOption={false}>
                                    ...
                                    <Col span={24}>
                                        <Form.Item style={{ margin: 0 }} name={[index, "shelf"]}>
                                            <Select disabled={true} size="large" showSearch placeholder="ชั้นวาง" filterOption={false}>
                        ...
                        <Row gutter={[0, 10]}>
                            <Col span={24}>
                                <Form.Item style={{ margin: 0 }} name={[index, "amount"]}>
                                    <InputNumber disabled={true} size="large" style={{ width: "100%" }} stringMode min={1}
                            ...
                            <Col span={24}>
                                <Form.Item noStyle name={[index, "purchase_unit_id"]}>
                                    <Select disabled={true} placeholder="หน่วยซื้อ" size="large" showSearch dropdownMatchSelectWidth={false} style={{ width: "100%", margin: 0, padding: 0, overflow: "hidden", textAlign: "center" }}>
                        ...
                        <Row gutter={[0, 10]} hidden={isDiscountAll}>
                            <Col span={24}>
                                <Badge count={
                                    ...
                                    <Form.Item style={{ margin: 0 }} name={[index, "price_discount"]}>
                                        <InputNumber disabled={true} size="large" style={{ width: "100%" }} stringMode min={0} step={"1"} precision={2}
                                            placeholder={"บาท"}
                                            formatter={(value) => !!value && value.length > 0 ? `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',') : ""}
                                            parser={(value) => value.replace(/\$\s?|(,*)/g, '')}
                            ...
                            <Col span={24}>
                                <Form.Item style={{ margin: 0 }} name={[index, "price_discount_percent"]}>
                                    <InputNumber disabled={true} size="large" style={{ width: "100%" }} stringMode min={0} step={"0.01"} precision={2}
                                        placeholder={"%"}
                                        formatter={(value) => !!value && value.length > 0 ? `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',') : ""}
                                        parser={(value) => value.replace(/\$\s?|(,*)/g, '')}
                        ...
                        <Popover content={
                            <>
                                <Form.Item name={[index, "remark"]} style={{ margin: 0 }}>
                                    <Input.TextArea onChange={(value) => debounceListProductRemark(value.target.value, index)} rows={10} disabled={true} />
                                </Form.Item>
                            </>
                        } trigger="click">
    ...
    const callbackSelectProduct = (value, index1, index2, amount, rowIndex) => {
        try {
            ...
            const { product_list, product_cost_product_stocks } = value
            const shop_stock_id = list_service_product[rowIndex]["shop_stock_id"]
