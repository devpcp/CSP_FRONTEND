...
const TemporaryDeliveryOrderDoc = ({ docTypeId }) => {
    ...
    * */
    ...
    const setColumnsTable = (searchStatus) => {
        ...
        _column.push(
            ...
            // },
            {
                title: () => GetIntlMessages("เลขทะเบียนรถยนต์"),
            ...
            },
            {
                ...
                render: (text, record) => {
                    if (
                        ...
                        return (
                            <PrintOut textButton={"พิมพ์"} printOutHeadTitle={"ใบเสร็จรับเงิน/ใบกำกับภาษี"} documentId={record?.id} style={{ width: "100%", display: "flex", justifyContent: "center", alignItems: "center", overflow: "hidden" }} customPriceUse={true} docTypeId={record.is_inv ? "e67f4a64-52dd-4008-9ef0-0121e7a65d48" : "b39bcb5d-6c72-4979-8725-c384c80a66c3"} settingPrint={settingPrint} />
                        )
                    }
                },
            },
    ...
    const getDataSearch = async ({ search = modelSearch.search ?? "", limit = configTable.limit, page = configTable.page, sort = configSort.sort, order = (configSort.order === "descend" ? "desc" : "asc"), _status = "active", doc_date_startDate = isArray(modelSearch.select_date) ? modelSearch.select_date[0] ?? "" : null, doc_date_endDate = isArray(modelSearch.select_date) ? modelSearch.select_date[1] ?? "" : null, payment_paid_status = modelSearch.payment_paid_status }) => {
        try {
            ...
            const res = await API.get(`/shopTaxInvoiceDoc/all?search=${search}&status=${_status}&page=${page}&limit=${limit}&sort=${sort}&order=${order}&ShopServiceOrderDoc__doc_sales_type=1${doc_date_startDate ? `&doc_date_startDate=${moment(doc_date_startDate).format("YYYY-MM-DD")}` : ""}${doc_date_endDate ? `&doc_date_endDate=${moment(doc_date_endDate).format("YYYY-MM-DD")}` : ""}${payment_paid_status ? `&ShopServiceOrderDoc__payment_paid_status=${payment_paid_status}` : ""}`)
                        ...
                        <div style={{ width: "100%" }}>
                            <Row gutter={[10, 10]} justify="end">
                                ...
                                <Col xxl={4} lg={6} md={12} xs={24} hidden={configModal.mode === "add" || form.getFieldValue("status") != 1} >
                                    <PrintOut textButton={"พิมพ์ใบเสร็จรับเงิน"} printOutHeadTitle={"ใบเสร็จรับเงิน/ใบกำกับภาษี"} loading={loading || carPreLoading} documentId={form.getFieldValue("id")} style={{ width: "100%", display: "flex", justifyContent: "center", alignItems: "center", overflow: "hidden" }} customPriceUse={true} docTypeId={form.getFieldValue().doc_type_id} />
                                </Col>
                                ...
                                <Col xxl={4} lg={6} md={12} xs={24} hidden={configModal.mode === "add" || form.getFieldValue("is_draft") === true || form.getFieldValue("status") != 1} >
                                    <PaymentDocs loading={loading || carPreLoading} docId={form.getFieldValue("id")} title={`ชำระเงิน ใบเสร็จรับเงิน/ใบกำกับภาษี ${form.getFieldValue("inv_code_id") ?? form.getFieldValue("abb_code_id")}`} handleCancelTaxDoc={handleCancel} initForm={form} setCarPreLoading={setCarPreLoading} />
                                </Col>
