...
const TemporaryDeliveryOrderDoc = ({ docTypeId, title = null, callBack, }) => {
    ...
    const [paymentPaidStatus, setPaymentPaidStatus] = useState(1)
    ...
    * */
    const configSearch = {
        search: [
            ...
            },
            {
                ...
                type: "select",
                name: "status",
                label: GetIntlMessages("สถานะใบส่งสินค้าชั่วคราว"),
    ...
    const setColumnsTable = (searchStatus) => {
        ...
        _column.push(
            ...
            },
            {
                ...
                align: "center",
                render: (text, record) => {
                    let payment_paid_status = record?.ShopServiceOrderDoc?.payment_paid_status
                    return (
                        <PrintOut customPriceUse={1} customFootSign={(authUser?.UsersProfile?.ShopsProfile?.id === "db945efe-17c8-4c43-a437-31204fe3b8af") ? { left: `ผู้รับสินค้า`, right: `ผู้ส่งสินค้า` } : null} textButton={"พิมพ์"} printOutHeadTitle={(authUser?.UsersProfile?.ShopsProfile?.id === "1a523ad4-682e-4db2-af49-d54f176a84ad") ? payment_paid_status === 1 || payment_paid_status === 2 ? "ใบส่งสินค้า/ใบแจ้งหนี้" : `ใบส่งสินค้า` : (authUser?.UsersProfile?.ShopsProfile?.id === "db945efe-17c8-4c43-a437-31204fe3b8af") ? "ใบส่งสินค้าชั่วคราว" : "ใบส่งสินค้า/ใบแจ้งหนี้"} documentId={record?.id} style={{ width: "100%", display: "flex", justifyContent: "center", alignItems: "center", overflow: "hidden" }} docTypeId={docTypeId} settingPrint={settingPrint} />
                    )
                },
            },
            {
    ...
    const getDataSearch = async ({ search = modelSearch.search ?? "", limit = configTable.limit, page = configTable.page, sort = configSort.sort, order = (configSort.order === "descend" ? "desc" : "asc"), _status = "active", doc_date_startDate = isArray(modelSearch.select_date) ? modelSearch.select_date[0] ?? "" : null, doc_date_endDate = isArray(modelSearch.select_date) ? modelSearch.select_date[1] ?? "" : null, payment_paid_status = modelSearch.payment_paid_status }) => {
        try {
            ...
            // const res = await API.get(`/shopServiceOrderDoc/all?search=${search}&status=${_status}&page=${page}&limit=${limit}&sort=${sort}&order=${order}`)
            const res = await API.get(`/shopTemporaryDeliveryOrderDoc/all?search=${search}&status=${_status}&page=${page}&limit=${limit}&sort=${sort}&order=${order}&ShopServiceOrderDoc__doc_sales_type=1${doc_date_startDate ? `&doc_date_startDate=${moment(doc_date_startDate).format("YYYY-MM-DD")}` : ""}${doc_date_endDate ? `&doc_date_endDate=${moment(doc_date_endDate).format("YYYY-MM-DD")}` : ""}${payment_paid_status ? `&ShopServiceOrderDoc__payment_paid_status=${payment_paid_status}` : ""}`)
            ...
            if (res.data.status === "success") {
                ...
                data = data.map(e => {
                    ...
                    const find = !!ShopTaxInvoiceDocs ? ShopTaxInvoiceDocs.find(where => where.shop_service_order_doc_id === shop_service_order_doc_id && where.status === 1) : null
    ...
    const setFormValueData = (value) => {
        try {
            ...
            setPaymentPaidStatus(ShopServiceOrderDoc.payment_paid_status)
                ...
                await onFinish(form.getFieldValue(), true).then(async (result) => {
                    if (result.data.status === "success") {
                        ...
                        if (data.status === "success") {
                            ...
                            const model = { ...ShopTemporaryDeliveryOrderDoc.current, doc_date, doc_type_id: docTypeId, list_service_product }
                            form.setFieldsValue({ ...model })
                        } else {
                            Swal.fire({ title: 'มีบางอย่างผิดพลาดกรุณาติอต่อเจ้าหน้าที่ !!', icon: 'error', confirmButtonText: GetIntlMessages("submit"), confirmButtonColor: mainColor })
                        }
        ...
        try {
            ...
            const model = {
                ...
                details: {
                    ...
                    upload_car_list: await values.upload_car_list.fileList === undefined ? await values.upload_car_list : values.upload_car_list.fileList,
            ...
            console.log('model.shopTemporaryDeliveryOrderLists :>> ', model.shopTemporaryDeliveryOrderLists);
            const invalidList = []
    ...
    /*end invoices button*/
    // console.log('form.getFieldValue("is_draft") :>> ', form.getFieldValue());
    ...
    return (
        <>
            ...
            {/* <TableList columns={columns} data={listSearchDataTable} loading={loading} configTable={configTable} callbackSearch={getDataSearch} addEditViewModal={addEditViewModal} changeStatus={changeStatus} isUseSwalFireOnDel={true} docTypeId={docTypeId} docStatus={modelSearch} isUseHandleBlockOnManage /> */}
            ...
            <ModalFullScreen
                ...
                title={<ModalFullScreenTitle title={`ใบส่งสินค้าชั่วคราว`} isShowTittle={true} />}
                        ...
                        <div style={{ width: "100%" }}>
                            <Row gutter={[10, 10]} justify="end">
                                ...
                                <Col xxl={4} lg={6} md={12} xs={24} hidden={configModal.mode === "add" || form.getFieldValue("status") != 1} >
                                    <PrintOut textButton={"พิมพ์ใบส่งสินค้า"} printOutHeadTitle={(authUser?.UsersProfile?.ShopsProfile?.id === "1a523ad4-682e-4db2-af49-d54f176a84ad") ? paymentPaidStatus === 1 || paymentPaidStatus === 2 ? "ใบส่งสินค้า/ใบแจ้งหนี้" : `ใบส่งสินค้า` : "ใบส่งสินค้า/ใบแจ้งหนี้"} loading={loading || carPreLoading} documentId={form.getFieldValue("id")} style={{ width: "100%", display: "flex", justifyContent: "center", alignItems: "center", overflow: "hidden" }} customPriceUse={true} docTypeId={docTypeId} />
                                </Col>
                                <Col xxl={4} lg={6} md={12} xs={24} hidden={configModal.mode !== "edit" || form.getFieldValue("is_draft") === false || form.getFieldValue("status") != 1} >
                                    <Button loading={loading || carPreLoading} onClick={() => confirmOrder()} type="text" style={{ overflow: "hidden", width: "100%", borderColor: "black", padding: 2 }}>{GetIntlMessages("ยืนยันใบส่งสินค้าชั่วคราว")}</Button>
                                </Col>

                                {/* <Col xxl={4} lg={6} md={12} xs={24} hidden={configModal.mode === "add" || form.getFieldValue("is_draft") === true} > */}
            ...
            >
                ...
                >
                    ...
                    <Form.Item name="upload_car_list" hidden>

                    </Form.Item>
                    <Form.Item name="upload_product_list" hidden>

                    </Form.Item>
                    <Form.Item name="upload_payment_list" hidden>
...
export default TemporaryDeliveryOrderDoc