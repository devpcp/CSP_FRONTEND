...
import { PlusOutlined, DeleteOutlined, FormOutlined, CalculatorOutlined, CheckCircleFilled, SyncOutlined } from "@ant-design/icons";
...
const ComponentsRoutesModalTab1ServiceAndProductV2 = ({ onFinish, calculateResult, mode }) => {
        ...
        const _column = [
            ...
            },
            {
                ...
                render: (text, record, index) => (
                    <>
                        <Form.Item style={{ margin: 0 }} name={[index, "price_unit"]}>
                            <InputNumber disabled={mode === "view"} size="large" style={{ width: "100%" }} step="1" stringMode precision={2}
                                formatter={(value) => !!value && value.length > 0 ? `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',') : ""}
                                parser={(value) => value.replace(/\$\s?|(,*)/g, '')}
                                onBlur={(value) => calculateTable(takeOutComma(value.target.value), index, "price_unit")}
                                addonAfter={
    ...
    const getPriceArray = async () => {
        try {
            let product_cost
            const { list_service_product, tags_obj } = form.getFieldValue()
            const shop_stock_id = list_service_product[priceIndexSelect]["shop_stock_id"]

            let shopProductId = list_service_product[priceIndexSelect]["shop_product_id"]
            const { data } = await API.get(`/shopProducts/byid/${shopProductId}`);
            let price_arr = []
            let suggestedPrice = 0
            let suggestedPriceWholeSale = 0
            if (data.status === "success") {
                const _model = data.data[0];
                suggestedPrice = _model.price.suggasted_re_sell_price.retail
                suggestedPriceWholeSale = _model.price?.suggasted_re_sell_price?.wholesale
                price_arr = _model.price_arr ?? []
            } else {
            ...
            console.log("data", data)
            // const suggestedPrice = list_service_product[priceIndexSelect]["shop_stock_list"].find(where => where.id === shop_stock_id).ShopProduct?.price?.suggasted_re_sell_price?.retail ?? null
            // const suggestedPriceWholeSale = list_service_product[priceIndexSelect]["shop_stock_list"]?.find(where => where.id === shop_stock_id)?.ShopProduct?.price?.suggasted_re_sell_price?.wholesale ?? null;
            // const { data } = await API.get(`/shopProducts/byid/${id}`);
            console.log("sss", list_service_product[priceIndexSelect]["shop_stock_list"])




            // price_arr = list_service_product[priceIndexSelect]["shop_stock_list"]?.find(where => where.id === shop_stock_id)?.ShopProduct?.price_arr ?? []
            price_arr.map((e) => {
                e.selected = false
            })
            ...
            list_service_product[priceIndexSelect]["price_arr"] = price_arr
                            ...
                            children: (
                                <>
                                    <div style={{ textAlign: "end" }}>
                                        <Button onClick={() => getPriceArray()}><SyncOutlined /></Button>
                                    </div>
