...
const ComponentsRoutesDocumentDebtLists = ({ onFinish, calculateResult, mode }) => {
    ...
    const selectItem = (value, index) => {
        try {
            ...
            const { price_unit, amount } = find
            const price_grand_total = Number(price_unit) * Number(amount)
            if (find) {
                arr_debt_list[index] = { ...find, price_grand_total, list_id: value, list_name: value }
