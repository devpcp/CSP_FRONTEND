...
import { Button, Col, Divider, Form, InputNumber, message, Row, Table, Input } from 'antd'
    ...
    */
    ...
    const setColumnsTable = (docSaleType) => {
        ...
        const debtColumn = [
            ...
            },
            {
                ...
                width: "10%",
                align: "center",
                // render: (text, record) => console.log('record :>> ', Number(form.getFieldValue("customer_credit_debt_payment_period"))),
                render: (text, record) => moment(moment(record?.doc_date).add(Number(form.getFieldValue("customer_credit_debt_payment_period")), 'd')).format("YYYY-MM-DD"),
    ...
    const handleAddDebtDoc = (value) => {
        try {
            ...
            const isObj = isPlainObject(shopCustomerDebtLists?.find(where => (where.is_draft === false ? where.id : where?.ShopServiceOrderDoc?.id) === value.id));

            let arr, newValue = { ...value, debt_due_date: moment(value.doc_date).add(Number(value[customer_type === "person" ? "ShopPersonalCustomer" : "ShopBusinessCustomer"]?.["other_details"]?.credit_term) ?? 0, 'd') ?? null };
            if (isObj) {
        ...
        try {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    console.log('arr :>> ', arr);
                    const test = { shopCustomerDebtLists: arr }
                    form.setFieldValue("shopCustomerDebtLists", arr)
                    console.log('form.getFieldValue("shopCustomerDebtLists") :>> ', form.getFieldValue());
                    // setDebtListTable((prevValue) => {
                    //     return prevValue.filter(where => where.id !== value.id)
                    // })
                }
            })
