            ...
            switch (type) {
                case "search":
                    if (!!value) {
                        ...
                        const { data } = await API.get(`/shopTemporaryDeliveryOrderDoc/all?is_draft=not_draft&status=active&ShopServiceOrderDoc__is_draft=not_draft&page=1&limit=50&sort=doc_date&order=desc&ShopServiceOrderDoc__doc_sales_type=${doc_sales_type}`)
            ...
            <Row gutter={[20, 0]}>
                {mode !== "view" ?
                    <>
                        ...
                        <Col span={12}>
                            <Form.Item name="doc_sales_type" label={"ประเภทการค้นหา"} style={{ width: "20%" }}>
                                <Select>
