    ...
    * */
    const configSearch = {
        ...
        button: {
            create: true,
            name: {
                add: GetIntlMessages(`สร้าง${`ใบเพิ่มหนี้ลูกหนี้` ?? documentTypesName}`),
            },
            download: false,
            import: false,
            export: false,
    ...
    const getDataSearch = async ({ search = modelSearch.search ?? "", limit = configTable.limit, page = configTable.page, sort = configSort.sort, order = (configSort.order === "descend" ? "desc" : "asc"), _status = modelSearch?.status ?? "active" }) => {
        try {
            ...
            const res = await API.get(`/shopCustomerDebtDebitNoteDoc/all?search=${search}&status=${_status}&page=${page}&limit=${limit}&sort=${sort}&order=${order}`)
        ...
        try {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    setLoading(true)
                    const { data } = await API.put(`/shopCustomerDebtDebitNoteDoc/put/${id}`, { status })
    ...
    const addEditViewModal = async (mode, id, isPayment, isInovices) => {
        try {
            ...
            if (id) {
                const { data } = await API.get(`/shopCustomerDebtDebitNoteDoc/byId/${id}`)
    ...
    const setFormValueData = async (value) => {
        try {
            ...
            const model = {
                ...
                options_list,
                arr_debt_list: ShopCustomerDebtDebitNoteLists,
    ...
    const onFinish = async (values) => {
        try {
            ...
            const model = {
                ...
                shopCustomerDebtDebitNoteLists: !!arr_debt_list && arr_debt_list.length > 0 ?
            ...
            if (configModal.mode === "add") {
                // res = await API.post(`https://2e0e-49-228-106-161.ngrok-free.app/api/shopCustomerDebtDebitNoteDoc/add`, model)
                res = await API.post(`/shopCustomerDebtDebitNoteDoc/add`, model)
            } else if (configModal.mode === "edit") {
                const { id } = form.getFieldValue()
                res = await API.put(`/shopCustomerDebtDebitNoteDoc/put/${id}`, model)
            }

            if (res.data.status === "success") {
                Swal.fire({
                    title: GetIntlMessages("บันทึกใบเพิ่มหนี้ลูกหนี้สำเร็จ !!"),
                    icon: "success",
                    timer: 2000
                })
                handleCancel()
    ...
    return (
        <>
            ...
            <ModalFullScreen
                ...
                title={<ModalFullScreenTitle title={`ใบเพิ่มหนี้ลูกหนี้` ?? documentTypesName} isShowTittle={true} />}
                loading={loading}
                CustomsButton={() => {
                    return (
